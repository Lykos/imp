...
class SyntacticParser
  ...
  def parse_statement
    ...
    if statement_continues?
      ...
      statement = CompositeStatement.new statements
    end
    statement
  end
  ...
  def parse_simple_statement
    ...
    if t =~ IF_K
      ...
      parse_end
      Conditional.new(condition, then_part, else_part)
    elsif t =~ WHILE_K
      parse_while
      condition = parse_boolean_expression
      parse_do
      ...
      parse_until
      condition = parse_boolean_expression
      CompositeStatement.new([body, WhileLoop.new(Not.new(condition), body)])
    elsif t =~ SKIP
      parse_skip
      Skip.new
    elsif t =~ VAR
      parse_var
      variable = Variable.new(parse_identifier)
      parse_assignment
      value = parse_arithmetic_expression
      parse_in
      body = parse_statement
      parse_end
      LocalVariableDeclaration.new(variable, value, body)
    elsif t =~ IDENTIFIER
      variable = Variable.new(parse_identifier)
      parse_assignment
      value = parse_arithmetic_expression
  ...
  def parse_boolean_expression
    ...
    if t =~ NOT_K
      ...
      begin
        ...
        if operator =~ OR_K
          Or.new(left_expression, right_expression)
        elsif operator =~ AND_K
          And.new(left_expression, right_expression)
        else
  ...
  def parse_arithmetic_expression
    ...
    if t =~ PARENTHESE_OPEN
      ...
      parse_parenthese_closed
      if operator =~ PLUS
        Plus.new(left_expression, right_expression)
      elsif operator =~ MINUS
        Minus.new(left_expression, right_expression)
      elsif operator =~ TIMES
        Times.new(left_expression, right_expression)
      else
  ...
  def parse_simple_arithmetic_expression
    t = look_ahead
    if t =~ IDENTIFIER
      Variable.new(parse_identifier)
    elsif t =~ NUMERAL
      IntegerConstant.new(parse_numeral)
    end
  end
end
